...
def combine_all_symbols(csv_dir, start_date, end_date, start_date_override=None):
...
def save_all_results(df_target_history, df_target_rt, raw_df, df_target, output_dfs, debug_data_dir, output_dir):
...
if __name__ == "__main__":
    ...
    if first_run == True:
        ...
        # drop any rows with duplicate index
        ...
        save_all_results(df_target_history, df_target_rt, raw_df, df_target, output_dfs, config.debug_data_dir, config.output_dir)

    else: 
        # 读取上次处理的结果
        df_target = pd.read_parquet(os.path.join(config.debug_data_dir, "funding_5min_combined.parquet"))
        last_date_in_history, last_label_in_history = df_target.index.get_level_values("date").max(), df_target.index.get_level_values("Label").max()
        print(f"last_date_in_history: {last_date_in_history}, last_label: {last_label_in_history}")
